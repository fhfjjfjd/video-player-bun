...
backend language has a compiler installed on this machine and compiling if it
does, (2) updating the documentation, (3) committing, (4) pushing, (5) bumping
the version, (6) tagging, and (7) creating a GitHub release. There is NO
GitHub Actions trigger, NO artifact download/verification, and NO binary
upload. A release is created from docs and version changes only and does not
attach binaries. If the backend language has no compiler on this machine, skip
compilation entirely and just publish as normal.
...
2.1. **Determine the backend language.** Look at the backend source directory
     (currently `src/server/`) and find its language marker:
     `Cargo.toml` (Rust), `go.mod` (Go), `pyproject.toml` / `requirements.txt`
     (Python), `package.json` (Node), a `cpp/` directory (C++), or a `php/`
     directory (PHP). If there is no backend at all, skip this phase.

2.2. **Check for a compiler/toolchain on this machine.** For the detected
     language, check whether the toolchain is installed, e.g.:
     `command -v <compiler>` — `g++`/`clang++` (C++), `cargo` (Rust), `go`
     (Go), `python3` (Python), `bun`/`node` (Node), `php` (PHP), etc.

2.3. **If the compiler IS installed → compile on this machine.** Build the
     backend locally using the project's build entry point (`bash build.sh`,
     or the language's standard build command). For interpreted languages
     (PHP), there is nothing to compile — instead run a syntax check
     (`php -l`) on the backend files. If the build/lint fails, fix the code
     and retry; do not proceed to a release on a failed build.

2.4. **If the compiler is NOT installed → skip compilation.** Do NOT install
     a compiler, do NOT trigger CI to build, and do NOT worry about missing
     binaries. Just proceed to the rest of the workflow ("đăng như bình
     thường").
...
4.1. **Read the current version** in `package.json` (`"version"` field).
...
4.2. **Compute the target version.** It must be STRICTLY HIGHER than the
     ...
     - If the previous tag was never reflected in `package.json` (the file
       lags behind the latest tag), still bump from the TAG, not from the
       file.

4.3. **Apply the bump** by editing the `version` field in `package.json`.
...
8.2. Record the release URL (from the `gh release create` output or
     `gh release view v<version> --repo fhfjjfjd/video-player-bun --json url
     --jq .url`).
...
3.1. **Compile locally ONLY when a compiler exists.** The only build that ever
     happens is Phase 2 on this machine, and only when the backend language's
     compiler/toolchain is installed (`command -v <compiler>` succeeds). If
     the compiler is missing, never install one, never trigger CI, and never
     fail the task — just publish as normal.
